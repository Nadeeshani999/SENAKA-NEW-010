...
async function initTenderPage() {
  ...
  function renderView(rows) {
    ...
    /* ── Delay Days Meters — one per tender (completed H col + ongoing L col) ── */
    // ── Delay meters: split into On-Going (left) and Completed (right) ──
    const delayOngoingEl=document.getElementById('tender-delay-ongoing');
    const delayCompletedEl=document.getElementById('tender-delay-completed');
    if (delayOngoingEl||delayCompletedEl) {
      const delayRows=rows.filter(r=>!isNaN(parseFloat(r['Delays (Expected or Actual)']??r['Delays']??'')));
      const ongoingDelayRows=delayRows.filter(r=>r._tabStatus==='On-Going');
      const completedDelayRows=delayRows.filter(r=>r._tabStatus==='Completed');
      ...
      if (!document.getElementById('delay-meter-style')) {
        ...
        dst.textContent=`
          ...
          .delay-meter-card { text-align:center; min-width:140px; }
          .delay-meter-card.delayed { animation:delayPulse 1.3s ease-in-out infinite; }
        `;
        document.head.appendChild(dst);
      }

      // Shared max for consistent needle scale across both groups
      const allDelayVals=delayRows.map(r=>Math.abs(parseFloat(r['Delays (Expected or Actual)']??r['Delays']??0)||0));
      const maxAbs=Math.max(1,...allDelayVals);

      let hasDelayed=false;
      const delayedTenders=[];

      function buildMeterHTML(r,i) {
        const delayVal=parseFloat(r['Delays (Expected or Actual)']??r['Delays']??0)||0;
        const isDelayed=delayVal>0;
        const isAhead=delayVal<0;
        if (isDelayed) { hasDelayed=true; delayedTenders.push(r['Tender No']||'Tender'); }

        const col=isDelayed?C.red:isAhead?C.green:C.muted;
        const fillPct=Math.min(100,(Math.abs(delayVal)/maxAbs)*100);
        ...
        const needleLen=rad-6;
        const ntx=cx+needleLen*Math.cos(needleAngle), nty=cy-needleLen*Math.sin(needleAngle);
        const bx=cx+(needleLen-7)*Math.cos(needleAngle), by=cy-(needleLen-7)*Math.sin(needleAngle);
        const px=Math.sin(needleAngle)*5, py=Math.cos(needleAngle)*5;
        const ticks=[25,50,75,100].map(tp=>{
          const ta=Math.PI-(tp/100)*Math.PI;
          const x1=cx+(rad-7)*Math.cos(ta), y1=cy-(rad-7)*Math.sin(ta);
          const x2=cx+(rad+2)*Math.cos(ta), y2=cy-(rad+2)*Math.sin(ta);
          return `<line x1="${x1.toFixed(1)}" y1="${y1.toFixed(1)}" x2="${x2.toFixed(1)}" y2="${y2.toFixed(1)}" stroke="rgba(255,255,255,.15)" stroke-width="1.2"/>`;
        }).join('');
        const gmId=`gm-delay-${i}`;
        const statusLabel=isDelayed?`⚠ +${Math.round(delayVal)}d DELAYED`:isAhead?`✅ ${Math.round(delayVal)}d ahead`:`✔ On schedule`;

        return `<div class="delay-meter-card${isDelayed?' delayed':''}" title="${r['Tender No']||''} · Delay: ${delayVal}d">
          <svg viewBox="0 0 120 72" width="140" height="84" style="overflow:visible">
            <defs>
              <linearGradient id="${gmId}" x1="0%" y1="0%" x2="100%" y2="0%">
                <stop offset="0%" stop-color="${col}" stop-opacity="0.2"/>
                <stop offset="100%" stop-color="${col}" stop-opacity="1"/>
              </linearGradient>
              ${isDelayed?`<filter id="dgl-${i}"><feGaussianBlur stdDeviation="2.5" result="cb"/><feMerge><feMergeNode in="cb"/><feMergeNode in="SourceGraphic"/></feMerge></filter>`:''}
            </defs>
            <path d="${bgArc}" fill="none" stroke="rgba(255,255,255,0.06)" stroke-width="11" stroke-linecap="round"/>
            ${ticks}
            ${fillArc?`<path d="${fillArc}" fill="none" stroke="url(#${gmId})" stroke-width="11" stroke-linecap="round" ${isDelayed?`filter="url(#dgl-${i})"`:''}/>`:''}
            <path d="M ${cx} ${cy} L ${bx.toFixed(2)} ${by.toFixed(2)}" stroke="${col}" stroke-width="2" stroke-linecap="round" opacity="0.85"/>
            <path d="M ${ntx.toFixed(2)},${nty.toFixed(2)} L ${(bx-px).toFixed(2)},${(by+py).toFixed(2)} L ${(bx+px).toFixed(2)},${(by-py).toFixed(2)} Z" fill="${col}" ${isDelayed?`filter="url(#dgl-${i})"`:''} style="filter:drop-shadow(0 0 ${isDelayed?5:2}px ${col})"/>
            <circle cx="${cx}" cy="${cy}" r="4" fill="${col}" stroke="rgba(0,0,0,.5)" stroke-width="1.5"/>
            <text x="60" y="52" text-anchor="middle" font-size="13" font-weight="800" fill="${col}" font-family="monospace">${isDelayed?'+':''}${Math.round(delayVal)}d</text>
            <text x="60" y="63" text-anchor="middle" font-size="7" fill="rgba(255,255,255,.4)" font-family="sans-serif">${isDelayed?'delay days':isAhead?'ahead':'on schedule'}</text>
          </svg>
          <div style="font-size:.63rem;font-weight:700;color:${col};margin-top:-.15rem;letter-spacing:.4px">${statusLabel}</div>
          <div style="font-size:.61rem;color:rgba(255,255,255,.55);margin-top:.15rem;font-weight:600">${r['Tender No']||'—'}</div>
          <div style="font-size:.57rem;color:rgba(255,255,255,.32);margin-top:.08rem">${r['Site Name']||'—'}</div>
        </div>`;
      }

      const emptyMsg='<div style="color:rgba(255,255,255,.28);font-size:.73rem;padding:.5rem 0">No data</div>';
      if (delayOngoingEl)   delayOngoingEl.innerHTML  = ongoingDelayRows.length   ? ongoingDelayRows.map((r,i)=>buildMeterHTML(r,i)).join('')                               : emptyMsg;
      if (delayCompletedEl) delayCompletedEl.innerHTML= completedDelayRows.length ? completedDelayRows.map((r,i)=>buildMeterHTML(r,i+ongoingDelayRows.length)).join('') : emptyMsg;

      if (hasDelayed) {
        setTimeout(()=>Sound.warn(), 800);
        toast(`⚠ DELAY ALERT: ${delayedTenders.join(', ')} — production behind schedule!`,'warn');
      }
    }
    ...
    rows._renderer=r=>{
      ...
      // Delay days from sheet (positive = delayed, negative = ahead)
      ...
      const delayIcon=isDelayed?'⚠ ':'';
      const delayTxt=hasDelay
        ? `${delayIcon}${isDelayed?'+':''}${Math.round(delayVal)}d`
        : '—';
      ...
      return `<tr${isDelayed?' class="row-expired"':''}>
        ...
        <td class="num ${delayCls}" style="font-weight:700;font-size:.82rem">${isDelayed?`<span style="animation:expiredPulse 1s infinite">${delayTxt}</span>`:delayTxt}</td>
